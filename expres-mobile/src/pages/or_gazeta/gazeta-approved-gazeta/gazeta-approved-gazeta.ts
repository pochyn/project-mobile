...
@Component({
  selector: 'page-gazeta-approved-gazeta',
  templateUrl: 'gazeta-approved-gazeta.html',
})
export class GazetaApprovedGazetaPage {
  ...
  getPosts(){
    ...
    queryRef2.get().then((snapShot) => {
      ...
      }).map(posts => posts.filter(post => (post.data.gazeta_type && post.data.checked_gazeta && !post.data.archieved_or  && !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site) || (post.data.nascrizna == true && this.nascriznyy && !post.data.archieved_or && post.data.checked_gazeta))));              
