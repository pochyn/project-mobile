...
@Component({
  selector: 'page-kv-new-regions',
  templateUrl: 'kv-new-regions.html',
})
export class KvNewRegionsPage {
  ...
  getPosts(){
    ...
    queryRef2.get().then((snapShot) => {
      ...
      }).map(posts => posts.filter(post => post.data.branch == br && post.data.regions_type && !post.data.checked && !post.data.read_kv && !post.data.archieved_kv&& !(post.data.mediaplan_gazeta || post.data.mediaplan_lviv || post.data.mediaplan_regions || post.data.mediaplan_site)));
